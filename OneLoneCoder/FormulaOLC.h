		...
		virtual bool OnUserUpdate(float fElapsedTime)
		{
			...
			for (int y = 0; y < ScreenHeight() / 2; y++)
			{
				for (int x = 0; x < ScreenWidth(); x++)
				{
					...
					float fRoadWidth = 0.1f + fPerspective * 0.9f; // Min 10% Max 90%
					float fClipWidth = fRoadWidth * 0.15f;
					fRoadWidth *= 0.5f; // Have it as track is symmetrical around center of track, but offset
					...
					// Using periodic oscillatory functions to give line, where the phase is controlled
